...
holding = pd.read_csv("../backtest_output/all_holding.csv", index_col='date')
...
riskfree = pd.read_csv("../database/riskfree.csv", sep='\t', index_col="Date",
                       usecols=["Date", "Price"])
riskfree.index = pd.to_datetime(riskfree.index, yearfirst=False)
riskfree.columns = ["riskfree"]
riskfree = riskfree / 1200.0

hedged = holding.merge(index, how="left", left_index=True, right_index=True)
...
hedged = hedged.merge(riskfree, how="left", left_index=True, right_index=True)
